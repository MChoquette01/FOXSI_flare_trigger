...
def graph_feature_importance(minutes_since_start, peak_filtering_minutes):
    ...
    f_i = sorted(f_i, key=lambda x: x[1], reverse=True)
    plt.figure(figsize=(16, 9))
    plt.bar([x for x in range(len(f_i))], [x[1] for x in f_i])
    plt.bar([x + len(f_i) for x in range(len(nans))], [x[1] for x in nans], color="red")
    plt.xticks(ticks=[x for x in range(len(f_i) + len(nans))], labels=[x[0] for x in f_i + nans], rotation=60, fontsize=14)
    plt.yticks(fontsize=14)
    plt.ylabel("Normalized Total Reduction of Split Criteria", fontsize=20)
